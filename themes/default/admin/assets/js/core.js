...
$(document).ready(function () {
    $(document).on('click', '.row_status', function (e) {
        ...
        if (row.hasClass('invoice_link')) {
            ...
            $('#myModal').modal('show');
        } else if (row.hasClass('facture_link')) {
            $('#myModal').modal({ remote: site.base_url + 'factures/update_status/' + id });
            $('#myModal').modal('show');
        } else if (row.hasClass('devis_link')) {
            $('#myModal').modal({ remote: site.base_url + 'devis/update_status/' + id });
            $('#myModal').modal('show');
...
function deadLineStatus(x) {
    if (x == null) {
        return '';
    } else if (x < 5) {
        return '<div class="text-center"><span style="padding: 5px 20px;" class="payment_status label label-success">' + x + '</span></div>';
    }else if (x < 10) {
        return '<div class="text-center"><span style="padding: 5px 20px; background: yellow; color: black;" class="payment_status label label-success">' + x + '</span></div>';
    }else if (x < 15) {
        return '<div class="text-center"><span style="padding: 5px 20px;" class="payment_status label label-warning">' + x + '</span></div>';
    }else if (x >= 20) {
        return '<div class="text-center"><span style="padding: 5px 20px; " class="payment_status label label-danger">' + x + '</span></div>';
    }
}
function formatSA(x) {
    x = x.toString();
...
$(document).ready(function () {
    ...
    $('body').on('click', '.customerCard_details_link td:not(:first-child, :last-child)', function () {
        $('#myModal').modal({
            remote: site.base_url + 'system_settings/view_customercard/' + $(this).parent('.customerCard_details_link').attr('id'),
        });
        $('#myModal').modal('show');
    });
    ...
    $('body').on('click', '.facture_link td:not(:first-child, :nth-last-child(3), :nth-last-child(2), :last-child)', function () {
        $('#myModal').modal({
            remote: site.base_url + 'factures/modal_view/' + $(this).parent('.facture_link').attr('id'),
        });
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'factures/view/' + $(this).parent('.facture_link').attr('id');
    });
    $('body').on('click', '.devis_link td:not(:first-child, :nth-last-child(3), :nth-last-child(2), :last-child)', function () {
        $('#myModal').modal({
            remote: site.base_url + 'devis/modal_view/' + $(this).parent('.devis_link').attr('id'),
        });
        $('#myModal').modal('show');
        //window.location.href = site.base_url + 'devis/view/' + $(this).parent('.devis_link').attr('id');
    });
    $('body').on('click', '.quote_link2', function () {
        $('#myModal').modal({ remote: site.base_url + 'quotes/modal_view/' + $(this).attr('id') });
        $('#myModal').modal('show');
    });
    $('body').on('click', '.facture_link2', function () {
        $('#myModal').modal({ remote: site.base_url + 'factures/modal_view/' + $(this).attr('id') });
        $('#myModal').modal('show');
    });
    $('body').on('click', '.devis_link2', function () {
        $('#myModal').modal({ remote: site.base_url + 'devis/modal_view/' + $(this).attr('id') });
        $('#myModal').modal('show');
    });
