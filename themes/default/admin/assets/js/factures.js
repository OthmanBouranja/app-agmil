$(document).ready(function () {
    ...
    if ((ftdiscount = localStorage.getItem('ftdiscount'))) {
        $('#ftdiscount').val(ftdiscount);
    }
    $('#fttax2').change(function (e) {
        localStorage.setItem('fttax2', $(this).val());
        $('#fttax2').val($(this).val());
    });
    if ((fttax2 = localStorage.getItem('fttax2'))) {
        $('#fttax2').select2('val', fttax2);
    }
    $('#ftstatus').change(function (e) {
        localStorage.setItem('ftstatus', $(this).val());
    });
    if ((ftstatus = localStorage.getItem('ftstatus'))) {
        $('#ftstatus').select2('val', ftstatus);
    }
    var old_shipping;
    $('#ftshipping')
        .focus(function () {
        ...
        .change(function () {
            ...
            localStorage.setItem('ftshipping', shipping);
            var gtotal = total + invoice_tax - order_discount + shipping;
            $('#gtotal').text(formatMoney(gtotal));
            $('#tship').text(formatMoney(shipping));
        });
    if ((ftshipping = localStorage.getItem('ftshipping'))) {
        shipping = parseFloat(ftshipping);
        $('#ftshipping').val(shipping);
    } else {
        shipping = 0;
    }

    $('#ftsupplier').change(function (e) {
        localStorage.setItem('ftsupplier', $(this).val());
        $('#supplier_id').val($(this).val());
    });
    if ((ftsupplier = localStorage.getItem('ftsupplier'))) {
        $('#ftsupplier')
            .val(ftsupplier)
            .select2({
    ...
    if (localStorage.getItem('ftitems')) {
        loadItems();
    }
    ...
    $('#reset').click(function (e) {
        bootbox.confirm(lang.r_u_sure, function (result) {
            if (result) {
                if (localStorage.getItem('ftitems')) {
                    localStorage.removeItem('ftitems');
                }
                if (localStorage.getItem('ftdiscount')) {
                    localStorage.removeItem('ftdiscount');
                }
                if (localStorage.getItem('fttax2')) {
                    localStorage.removeItem('fttax2');
                }
                if (localStorage.getItem('ftshipping')) {
                    localStorage.removeItem('ftshipping');
                }
                if (localStorage.getItem('ftref')) {
                    localStorage.removeItem('ftref');
                }
                if (localStorage.getItem('ftwarehouse')) {
                    localStorage.removeItem('ftwarehouse');
                }
                if (localStorage.getItem('ftnote')) {
                    localStorage.removeItem('ftnote');
                }
                if (localStorage.getItem('ftinnote')) {
                    localStorage.removeItem('ftinnote');
                }
                if (localStorage.getItem('ftcustomer')) {
                    localStorage.removeItem('ftcustomer');
                }
                if (localStorage.getItem('ftcurrency')) {
                    localStorage.removeItem('ftcurrency');
                }
                if (localStorage.getItem('ftdate')) {
                    localStorage.removeItem('ftdate');
                }
                if (localStorage.getItem('ftstatus')) {
                    localStorage.removeItem('ftstatus');
                }
                if (localStorage.getItem('ftbiller')) {
                    localStorage.removeItem('ftbiller');
                }
    ...
    $('#ftref').change(function (e) {
        localStorage.setItem('ftref', $(this).val());
    });
    if ((ftref = localStorage.getItem('ftref'))) {
        $('#ftref').val(ftref);
    }
    $('#ftwarehouse').change(function (e) {
        localStorage.setItem('ftwarehouse', $(this).val());
    });
    if ((ftwarehouse = localStorage.getItem('ftwarehouse'))) {
        $('#ftwarehouse').select2('val', ftwarehouse);
    }

    $('#ftnote').redactor('destroy');
    $('#ftnote').redactor({
        buttons: [
            'formatting',
            '|',
        ...
        changeCallback: function (e) {
            var v = this.get();
            localStorage.setItem('ftnote', v);
        },
    });
    if ((ftnote = localStorage.getItem('ftnote'))) {
        $('#ftnote').redactor('set', ftnote);
    }
    var $customer = $('#ftcustomer');
    $customer.change(function (e) {
        localStorage.setItem('ftcustomer', $(this).val());
    });
    if ((ftcustomer = localStorage.getItem('ftcustomer'))) {
        $customer.val(ftcustomer).select2({
            minimumInputLength: 1,
            data: [],
            initSelection: function (element, callback) {
                $.ajax({
    ...
    $('#ftdiscount')
        .focus(function () {
            old_ftdiscount = $(this).val();
        })
        .change(function () {
            var new_discount = $(this).val() ? $(this).val() : '0';
            if (is_valid_discount(new_discount)) {
                localStorage.removeItem('ftdiscount');
                localStorage.setItem('ftdiscount', new_discount);
                loadItems();
                return;
            } else {
                $(this).val(old_ftdiscount);
                bootbox.alert(lang.unexpected_value);
                return;
            }
        });
     ...
     * Delete Row Method
     * ---------------------- */
    $(document).on('click', '.ftdel', function () {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        delete ftitems[item_id];
        row.remove();
        if (ftitems.hasOwnProperty(item_id)) {
        } else {
            localStorage.setItem('ftitems', JSON.stringify(ftitems));
            loadItems();
            return;
        }
    });
        ...
        if (site.settings.tax1) {
            ...
            if (pr_tax !== null && pr_tax != 0) {
                $.each(tax_rates, function () {
                    if (this.id == pr_tax) {
                        if (this.type == 1) {
                            if (ftitems[item_id].row.tax_method == 0) {
    ...
    $(document).on('change', '#punit', function () {
        ...
        if (item.units && unit != ftitems[item_id].row.base_unit) {
            $.each(item.units, function () {
                if (this.id == unit) {
    ...
    $(document).on('click', '#editItem', function () {
        ...
        if (unit != ftitems[item_id].row.base_unit) {
            $.each(ftitems[item_id].units, function () {
                if (this.id == unit) {
                    base_quantity = unitToBaseQty($('#pquantity').val(), this);
                }
            });
        ...
        (ftitems[item_id].row.fup = 1),
            (ftitems[item_id].row.qty = parseFloat($('#pquantity').val())),
            (ftitems[item_id].row.base_quantity = parseFloat(base_quantity)),
            (ftitems[item_id].row.real_unit_price = price),
            (ftitems[item_id].row.unit = unit),
            (ftitems[item_id].row.tax_rate = new_pr_tax),
            (ftitems[item_id].tax_rate = new_pr_tax_rate),
            (ftitems[item_id].row.discount = $('#pdiscount').val() ? $('#pdiscount').val() : ''),
            (ftitems[item_id].row.option = $('#poption').val() ? $('#poption').val() : ''),
            (ftitems[item_id].row.serial = $('#pserial').val());
        localStorage.setItem('ftitems', JSON.stringify(ftitems));
        $('#prModal').modal('hide');
    ...
    $(document).on('click', '#addManually', function (e) {
        if (count == 1) {
            ftitems = {};
            if ($('#ftwarehouse').val() && $('#ftcustomer').val()) {
                $('#ftcustomer').select2('readonly', true);
                $('#ftwarehouse').select2('readonly', true);
            } else {
    ...
    $(document)
        ...
        .on('change', '.rquantity', function () {
            ...
            var new_qty = parseFloat($(this).val()),
                item_id = row.attr('data-item-id');
            ftitems[item_id].row.base_quantity = new_qty;
            if (ftitems[item_id].row.unit != ftitems[item_id].row.base_unit) {
                $.each(ftitems[item_id].units, function () {
                    if (this.id == ftitems[item_id].row.unit) {
                        ftitems[item_id].row.base_quantity = unitToBaseQty(new_qty, this);
                    }
                });
            }
            ftitems[item_id].row.qty = new_qty;
            localStorage.setItem('ftitems', JSON.stringify(ftitems));
            loadItems();
        });
    ...
    $(document)
        ...
        .on('change', '.rprice', function () {
            ...
            var new_price = parseFloat($(this).val()),
                item_id = row.attr('data-item-id');
            ftitems[item_id].row.price = new_price;
            localStorage.setItem('ftitems', JSON.stringify(ftitems));
            loadItems();
        });

    $(document).on('click', '#removeReadonly', function () {
        $('#ftcustomer').select2('readonly', false);
        //$('#ftwarehouse').select2('readonly', false);
        return false;
    });
});
/* -----------------------
...
function loadItems() {
    if (localStorage.getItem('ftitems')) {
        ...
        $('#ftTable tbody').empty();
        ftitems = JSON.parse(localStorage.getItem('ftitems'));
        sortedItems =
            site.settings.item_addition == 1
                ? _.sortBy(ftitems, function (o) {
        ...
        $.each(sortedItems, function () {
            ...
            tr_html +=
                ...
                '</span></td>';
            tr_html +=
                '<td class="text-center"><i class="fa fa-times tip pointer ftdel" id="' +
                row_no +
                '" title="Remove" style="cursor:pointer;"></i></td>';
            newTr.html(tr_html);
            newTr.prependTo('#ftTable');
...
function add_invoice_item(item) {
    ...
    var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
    if (ftitems[item_id]) {
        var new_qty = parseFloat(ftitems[item_id].row.qty) + 1;
        ftitems[item_id].row.base_quantity = new_qty;
        if (ftitems[item_id].row.unit != ftitems[item_id].row.base_unit) {
            $.each(ftitems[item_id].units, function () {
                if (this.id == ftitems[item_id].row.unit) {
        ...
        ftitems[item_id] = item;
    }
    ftitems[item_id].order = new Date().getTime();
    localStorage.setItem('ftitems', JSON.stringify(ftitems));
    loadItems();
    return true;
}
