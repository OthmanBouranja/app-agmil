$(document).ready(function() {
    ...
    if ((dvdiscount = localStorage.getItem('dvdiscount'))) {
        $('#dvdiscount').val(dvdiscount);
    }
    $('#dvtax2').change(function(e) {
        localStorage.setItem('dvtax2', $(this).val());
        $('#dvtax2').val($(this).val());
    });
    if ((dvtax2 = localStorage.getItem('dvtax2'))) {
        $('#dvtax2').select2('val', dvtax2);
    }
    $('#dvstatus').change(function(e) {
        localStorage.setItem('dvstatus', $(this).val());
    });
    if ((dvstatus = localStorage.getItem('dvstatus'))) {
        $('#dvstatus').select2('val', dvstatus);
    }
    var old_shipping;
    $('#dvshipping')
        .focus(function() {
        ...
        .change(function() {
            ...
            localStorage.setItem('dvshipping', shipping);
            var gtotal = total + invoice_tax - order_discount + shipping;
            $('#gtotal').text(formatMoney(gtotal));
            $('#tship').text(formatMoney(shipping));
        });
    if ((dvshipping = localStorage.getItem('dvshipping'))) {
        shipping = parseFloat(dvshipping);
        $('#dvshipping').val(shipping);
    } else {
        shipping = 0;
    }

    $('#dvsupplier').change(function(e) {
        localStorage.setItem('dvsupplier', $(this).val());
        $('#supplier_id').val($(this).val());
    });
    if ((dvsupplier = localStorage.getItem('dvsupplier'))) {
        $('#dvsupplier')
            .val(dvsupplier)
            .select2({
    ...
    $('#reset').click(function(e) {
        bootbox.confirm(lang.r_u_sure, function(result) {
            if (result) {
                ...
                if (localStorage.getItem('dvinnote')) {
                    localStorage.removeItem('dvinnote');
                }
    ...
    $('#dvref').change(function(e) {
        localStorage.setItem('dvref', $(this).val());
    });
    if ((dvref = localStorage.getItem('dvref'))) {
        $('#dvref').val(dvref);
    }
    $('#dvwarehouse').change(function(e) {
        localStorage.setItem('dvwarehouse', $(this).val());
    });
    if ((dvwarehouse = localStorage.getItem('dvwarehouse'))) {
        $('#dvwarehouse').select2('val', dvwarehouse);
    }

    $('#dvnote').redactor('destroy');
    $('#dvnote').redactor({
        buttons: [
            'formatting',
            '|',
        ...
        changeCallback: function(e) {
            var v = this.get();
            localStorage.setItem('dvnote', v);
        },
    });
    if ((dvnote = localStorage.getItem('dvnote'))) {
        $('#dvnote').redactor('set', dvnote);
    }
    var $customer = $('#dvcustomer');
    $customer.change(function(e) {
        localStorage.setItem('dvcustomer', $(this).val());
    });
    if ((dvcustomer = localStorage.getItem('dvcustomer'))) {
        $customer.val(dvcustomer).select2({
            minimumInputLength: 1,
            data: [],
            initSelection: function(element, callback) {
                $.ajax({
    ...
    $('#dvdiscount')
        .focus(function() {
            old_dvdiscount = $(this).val();
        })
        .change(function() {
            var new_discount = $(this).val() ? $(this).val() : '0';
            if (is_valid_discount(new_discount)) {
                localStorage.removeItem('dvdiscount');
                localStorage.setItem('dvdiscount', new_discount);
                loadItems();
                return;
            } else {
                $(this).val(old_dvdiscount);
                bootbox.alert(lang.unexpected_value);
                return;
            }
        });
     ...
     * Delete Row Method
     * ---------------------- */
    $(document).on('click', '.dvdel', function() {
        var row = $(this).closest('tr');
        var item_id = row.attr('data-item-id');
        delete dvitems[item_id];
        row.remove();
        if (dvitems.hasOwnProperty(item_id)) {
        } else {
            localStorage.setItem('dvitems', JSON.stringify(dvitems));
            loadItems();
            return;
        }
    });
        ...
        if (site.settings.tax1) {
            ...
            if (pr_tax !== null && pr_tax != 0) {
                $.each(tax_rates, function() {
                    if (this.id == pr_tax) {
                        if (this.type == 1) {
                            if (dvitems[item_id].row.tax_method == 0) {
    ...
    $(document).on('change', '#punit', function() {
        ...
        if (item.units && unit != dvitems[item_id].row.base_unit) {
            $.each(item.units, function() {
                if (this.id == unit) {
    ...
    $(document).on('click', '#editItem', function() {
        ...
        if (unit != dvitems[item_id].row.base_unit) {
            $.each(dvitems[item_id].units, function() {
                if (this.id == unit) {
                    base_quantity = unitToBaseQty($('#pquantity').val(), this);
                }
            });
        ...
        (dvitems[item_id].row.fup = 1),
            (dvitems[item_id].row.qty = parseFloat($('#pquantity').val())),
            (dvitems[item_id].row.base_quantity = parseFloat(base_quantity)),
            (dvitems[item_id].row.real_unit_price = price),
            (dvitems[item_id].row.unit = unit),
            (dvitems[item_id].row.tax_rate = new_pr_tax),
            (dvitems[item_id].tax_rate = new_pr_tax_rate),
            (dvitems[item_id].row.discount = $('#pdiscount').val() ? $('#pdiscount').val() : ''),
            (dvitems[item_id].row.option = $('#poption').val() ? $('#poption').val() : ''),
            (dvitems[item_id].row.serial = $('#pserial').val());
        localStorage.setItem('dvitems', JSON.stringify(dvitems));
        $('#prModal').modal('hide');
    ...
    $(document).on('click', '#addManually', function(e) {
        if (count == 1) {
            dvitems = {};
            if ($('#dvwarehouse').val() && $('#dvcustomer').val()) {
                $('#dvcustomer').select2('readonly', true);
                $('#dvwarehouse').select2('readonly', true);
            } else {
    ...
    $(document)
        ...
        .on('change', '.rquantity', function() {
            ...
            var new_qty = parseFloat($(this).val()),
                item_id = row.attr('data-item-id');
            dvitems[item_id].row.base_quantity = new_qty;
            if (dvitems[item_id].row.unit != dvitems[item_id].row.base_unit) {
                $.each(dvitems[item_id].units, function() {
                    if (this.id == dvitems[item_id].row.unit) {
                        dvitems[item_id].row.base_quantity = unitToBaseQty(new_qty, this);
                    }
                });
            }
            dvitems[item_id].row.qty = new_qty;
            localStorage.setItem('dvitems', JSON.stringify(dvitems));
            loadItems();
        });
    ...
    $(document)
        ...
        .on('change', '.rprice', function() {
            ...
            var new_price = parseFloat($(this).val()),
                item_id = row.attr('data-item-id');
            dvitems[item_id].row.price = new_price;
            localStorage.setItem('dvitems', JSON.stringify(dvitems));
            loadItems();
        });

    $(document).on('click', '#removeReadonly', function() {
        $('#dvcustomer').select2('readonly', false);
        //$('#dvwarehouse').select2('readonly', false);
        return false;
    });
});
/* -----------------------
...
function loadItems() {
    if (localStorage.getItem('dvitems')) {
        ...
        $('#dvTable tbody').empty();
        dvitems = JSON.parse(localStorage.getItem('dvitems'));
        sortedItems =
            site.settings.item_addition == 1
                ? _.sortBy(dvitems, function(o) {
        ...
        $.each(sortedItems, function() {
            ...
            tr_html +=
                ...
                '</span></td>';
            tr_html +=
                '<td class="text-center"><i class="fa fa-times tip pointer dvdel" id="' +
                row_no +
                '" title="Remove" style="cursor:pointer;"></i></td>';
            newTr.html(tr_html);
            newTr.prependTo('#dvTable');
...
function add_invoice_item(item) {
    ...
    var item_id = site.settings.item_addition == 1 ? item.item_id : item.id;
    if (dvitems[item_id]) {
        var new_qty = parseFloat(dvitems[item_id].row.qty) + 1;
        dvitems[item_id].row.base_quantity = new_qty;
        if (dvitems[item_id].row.unit != dvitems[item_id].row.base_unit) {
            $.each(dvitems[item_id].units, function() {
                if (this.id == dvitems[item_id].row.unit) {
        ...
        dvitems[item_id] = item;
    }
    dvitems[item_id].order = new Date().getTime();
    localStorage.setItem('dvitems', JSON.stringify(dvitems));
    loadItems();
    return true;
}
