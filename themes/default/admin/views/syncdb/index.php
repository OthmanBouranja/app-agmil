    ...
    ?>
        ...
        echo '<p class="text-danger">If you have added the new records, that will be updated too.</p>';
                    ...
                    <div class="form-group">
                     ...
                     <div class="form-group">
                        <div class="actions">
                            <div class="col-xs-3"><a href="#" class="btn btn-primary btn-block" id="update_factures">Update
                                    Factures Table</a></div>
                            <div class="performing" style="padding-top:7px;display:none;"><i
                                    class="fa fa-refresh fa-spin"></i></div>
                        </div>
                        <div class="clearfix"></div>
                    ...
                    <div class="form-group">
                        <div class="actions">
                            <div class="col-xs-3"><a href="#" class="btn btn-danger btn-block" id="reset_factures">Reset
                                    Factures Table</a></div>
                            <div class="performing" style="padding-top:7px;display:none;"><i
                                    class="fa fa-refresh fa-spin"></i></div>
                        </div>
...
<script type="text/javascript">
    $('document').ready(function () {
        ...
        $('#reset_factures').click(function (e) {
            var btn = $(this);
            e.preventDefault();
            bootbox.confirm("Are you sure to delete all the quotations and reset the table?", function (res) {
                if (res == true) {
                    ...
                    perf.show();
                    $.ajax({
                        url: '<?=admin_url('sync/reset_factures')?>',
                        type: 'GET',
                        success: function (msg) {
                            perf.html(msg);
                            btn.attr('disabled', false);
        ...
        $('#update_factures').click(function (e) {
            var btn = $(this);
            e.preventDefault();
            btn.attr('disabled', true);
            var perf = btn.parent().parent('.actions').children('.performing');
            perf.show();
            $.ajax({
                url: '<?=admin_url('sync/update_factures')?>',
                type: 'GET',
                success: function (msg) {
                    perf.html(msg);
                    btn.attr('disabled', false);
