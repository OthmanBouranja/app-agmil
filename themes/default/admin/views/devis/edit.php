...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        <?php if ($inv) {
    ?>
        localStorage.setItem('dvdate', '<?= date($dateFormats['php_ldate'], strtotime($inv->date))?>');
        localStorage.setItem('dvcustomer', '<?=$inv->customer_id?>');
        localStorage.setItem('dvbiller', '<?=$inv->biller_id?>');
        localStorage.setItem('dvsupplier', '<?=$inv->supplier_id?>');
        localStorage.setItem('dvref', '<?=$inv->reference_no?>');
        localStorage.setItem('dvwarehouse', '<?=$inv->warehouse_id?>');
        localStorage.setItem('dvstatus', '<?=$inv->status?>');
        localStorage.setItem('dvnote', '<?= str_replace(["\r", "\n"], '', $this->sma->decode_html($inv->note)); ?>');
        localStorage.setItem('dvdiscount', '<?=$inv->order_discount_id?>');
        localStorage.setItem('dvtax2', '<?=$inv->order_tax_id?>');
        localStorage.setItem('dvshipping', '<?=$inv->shipping?>');
        localStorage.setItem('dvitems', JSON.stringify(<?=$inv_items; ?>));
        <?php
} ?>
        <?php if ($Owner || $Admin) {
        ?>
        ...
        $('#edit_devis').click(function () {
            $(window).unbind('beforeunload');
            $('form.edit-dv-form').submit();
        });
    });
</script>
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-edit"></i><?= lang('edit_devis'); ?></h2>
    </div>
    <div class="box-content">
        <div class="row">
            <div class="col-lg-12">
                ...
                <?php
                $attrib = ['data-toggle' => 'validator', 'role' => 'form', 'class' => 'edit-dv-form'];
                echo admin_form_open_multipart('devis/edit/' . $id, $attrib)
                ?>
                ...
                <div class="row">
                    ...
                    ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('reference_no', (isset($_POST['reference_no']) ? $_POST['reference_no'] : ''), 'class="form-control input-tip" id="dvref" required="required"'); ?>
                            </div>
                        </div>
                    ...
                    echo form_dropdown('biller', $bl, (isset($_POST['biller']) ? $_POST['biller'] : $inv->biller_id), 'id="dvbiller" data-placeholder="' . $this->lang->line('select') . ' ' . $this->lang->line('biller') . '" required="required" class="form-control input-tip select" style="width:100%;"'); ?>
                                </div>
                            </div>
                        <?php
                } else {
                    ...
                    ?>
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <?php echo form_input('discount', '', 'class="form-control input-tip" id="dvdiscount" ' . (($Owner || $Admin || $this->session->userdata('allow_discount')) ? '' : 'readonly="true"')); ?>
                            </div>
                        </div>
                        <?php
                } ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang('shipping', 'dvshipping'); ?>
                                <?php echo form_input('shipping', '', 'class="form-control input-tip" id="dvshipping"'); ?>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <?= lang('supplier', 'dvsupplier'); ?>
                                <?php if ($Owner || $Admin || $GP['suppliers-add'] || $GP['suppliers-index']) {
                                    ?><div class="input-group"><?php
                                } ?>
                                    <input type="hidden" name="supplier" value="" id="dvsupplier"
                        ...
                        <div class="col-md-12">
                            <div class="panel panel-warning">
                                ...
                                <div class="panel-body" style="padding: 5px;">
                                    ...
                                    echo form_dropdown('warehouse', $wh, (isset($_POST['warehouse']) ? $_POST['warehouse'] : $inv->warehouse_id), 'id="dvwarehouse" class="form-control input-tip select" data-placeholder="' . $this->lang->line('select') . ' ' . $this->lang->line('warehouse') . '" required="required" style="width:100%;" '); ?>
                                            </div>
                                        </div>
                                    <?php
                                } else {
                        ...
                        <div class="row" id="bt">
                            <div class="col-sm-12">
                                <div class="col-sm-12">
                                    <div class="form-group">
                                        <?= lang('note', 'dvnote'); ?>
                                        <?php echo form_textarea('note', (isset($_POST['note']) ? $_POST['note'] : ''), 'class="form-control" id="dvnote" style="margin-top: 10px; height: 100px;"'); ?>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="col-sm-12">
                            <div
                                class="fprom-group"><?php echo form_submit('edit_devis', $this->lang->line('submit'), 'id="edit_devis" class="btn btn-primary" style="padding: 6px 15px; margin:15px 0;"'); ?>
