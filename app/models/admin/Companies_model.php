...
class Companies_model extends CI_Model
{
    ...
    public function getCustomerCardsuggestions($term, $limit = 10)
    {
        $this->db->select("id,address,city,gst_no,customer_group_name,customer_group_id,name,email,phone, (CASE WHEN company = '-' THEN name ELSE CONCAT(company, ' (', name, ')') END) as text, (CASE WHEN company = '-' THEN name ELSE CONCAT(company, ' (', name, ')') END) as value, phone", false);
        $this->db->where(" (id LIKE '%" . $term . "%' OR name LIKE '%" . $term . "%' OR company LIKE '%" . $term . "%' OR email LIKE '%" . $term . "%' OR phone LIKE '%" . $term . "%' OR vat_no LIKE '%" . $term . "%') ");
        $q = $this->db->get_where('customer_card', ['group_name' => ''], $limit);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
