...
class Settings_model extends CI_Model
{
    ...
    public function getCarBrandByID($id)
    {
        $q = $this->db->get_where('carbrands', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function updateCarBrand($id, $data = [])
    {
        if ($this->db->update('carbrands', $data, ['id' => $id])) {
            return true;
        }
        return false;
    }

    public function getParentCarBrands()
    {
        $this->db->where('parent_id', null)->or_where('parent_id', 0);
        $q = $this->db->get('carbrands');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    public function addCarBrand($data)
    {
        if ($this->db->insert('carbrands', $data)) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->delete('carbrands', ['id' => $id])) {
            return true;
        }
        return false;
    }
    ...
    {
        $q = $this->db->get_where('carmodels', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    {
        if ($this->db->update('carmodels', $data, ['id' => $id])) {
            return true;
        }
        return false;
    }

    public function getParentCarModels()
    {
        $this->db->where('parent_id', null)->or_where('parent_id', 0);
        $q = $this->db->get('carmodels');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
            }
    ...
    {
        if ($this->db->insert('carmodels', $data)) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->delete('carmodels', ['id' => $id])) {
            return true;
        }
        return false;
    }

    public function getCustomerCardByID($id)
    {
        $q = $this->db->get_where('customer_card', ['id' => $id], 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return false;
    ...
    public function updateCustomerCard($id, $data = [])
    {
        $dataCompanies = $data;
        if(isset($dataCompanies['company'])){
            $dataCompanies['company_name'] = $dataCompanies['company'];
            unset($dataCompanies['company']);
        }
        if ($this->db->update('customer_card', $data, ['id' => $id]) && $this->db->update('companies', $dataCompanies, ['customer_card' => $id])) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->insert('customer_card', $data)) {
            return true;
        }
        return false;
    }
    ...
    {
        if ($this->db->delete('customer_card', ['id' => $id])) {
            return true;
        }
        return false;
    }
    ...
    public function getModelsByBrand($carBrandId = null)
    {
        $this->db->select('id as id, name as text')
            ->where('parent_id', $carBrandId)->order_by('name');
        $q = $this->db->get('carbrands');
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
                $data[] = $row;
