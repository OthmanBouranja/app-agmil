...
class Quotes_model extends CI_Model
{
    ...
    public function addQuote($data = [], $items = [],$attachments = [])
    {
        if ($this->db->insert('quotes', $data)) {
            ...
            if (!empty($attachments)) {
                foreach ($attachments as $attachment) {
                    $attachment['subject_id']   = $quote_id;
                    $attachment['subject_type'] = 'quote';
    ...
    public function updateQuote($id, $data, $items = [],$attachments = [])
    {
    ...
    public function userReportSynthesis(){
        ...
        $this->db->select('status,count(1) as count');
        $this->db->where('created_by',$this->session->userdata('user_id'));
        $this->db->group_by('status');
        $userReportSynthesisList = $this->db->get('quotes')->result_array();

        foreach ($userReportSynthesisList as $userReportSynthesis){
            if($userReportSynthesis['status']==='En instance'){
                $template[0]['count'] = $userReportSynthesis['count'];
            }else{
                $template[0]['count'] = 0;
            }

            if($userReportSynthesis['status']==='En Production'){
                $template[1]['count'] = $userReportSynthesis['count'];
            }else{
                $template[1]['count'] = 0;
            }

            if($userReportSynthesis['status']==='Terminer'){
                $template[2]['count'] = $userReportSynthesis['count'];
            }else{
                $template[2]['count'] = 0;
            }
        }
    ...
    public function getByDevis($devis){
        $q = $this->db->get_where('quotes', ['devis' => $devis,'devis !='=>null]);
        if ($q->num_rows() > 0) {
            return true;
        }
        return false;
