...
class Customers extends MY_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run('customers/add') == true) {
            ...
            $customer_card_id = $this->input->post('customer_card');
            $customer_card = $this->companies_model->getCustomerCardByID($customer_card_id);


            $carBrand = null;
            $postal_code = $this->input->post('postal_code');
            if(isset($postal_code)){
                $carBrand = $this->settings_model->getCarBrandByID($postal_code);
            }

            $carModel = null;
            $country = $this->input->post('country');
            if(isset($country)){
                $carModel = $this->settings_model->getCarBrandByID($country);
            }
            ...
            $data = [
                ...
                'group_id'            => '3',
                'group_name'          => 'customer',
                'customer_group_id'   => $customer_card->customer_group_id,
                'customer_group_name' => $customer_card->customer_group_name,
                ...
                'company_name'         => $customer_card->company,
                'address'             => $customer_card->address,
                'vat_no'              => $this->input->post('vat_no'),
                'city'                => $customer_card->city,
                'state'               => $this->input->post('state'),
                'postal_code'         => $carBrand->name,
                'country'             => $carModel->name,
                'phone'               => $customer_card->phone,
        ...
        if ($this->form_validation->run() == true && $cid = $this->companies_model->addCompany($data)) {
            ...
            $this->data['m'] = "customers";
            $this->data['v'] = "add";
            $bc                                = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('customers'), 'page' => lang('customers')], ['link' => '#', 'page' => lang('Ajouter client')]];
            $meta                              = ['page_title' => lang('Ajouter client'), 'bc' => $bc];
            $this->page_construct('customers/add', $meta, $this->data);
        }
    }
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run('customers/add') == true) {
            ...
            if($carModel){
                $data['country'] = $carModel->name;
                $data['carModel'] = $carModel->id;
            }
        ...
        if ($this->form_validation->run() == true && $this->companies_model->updateCompany($id, $data)) {
            ...
            $this->data['customer_groups'] = $this->companies_model->getAllCustomerGroups();
            $this->data['price_groups']    = $this->companies_model->getAllPriceGroups();
            $bc                                = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('customers'), 'page' => lang('customers')], ['link' => '#', 'page' => lang('Modifier client')]];
            $meta                              = ['page_title' => lang('Modifier client'), 'bc' => $bc];
            $this->page_construct('customers/edit', $meta, $this->data);
        }
    }
    ...
    public function customerCardsuggestions($term = null, $limit = null, $a = null)
    {
        // $this->sma->checkPermissions('index');
        if ($this->input->get('term')) {
            $term = $this->input->get('term', true);
        ...
        $term   = addslashes($term);
        $limit  = $this->input->get('limit', true);
        $result = $this->companies_model->getCustomerCardsuggestions($term, $limit);
        if ($a) {
            $this->sma->send_json($result);
        }
        $rows['results'] = $result;
