...
class Sales extends MY_Controller
{
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $attachments        = $this->attachments->upload();
            $data['attachment'] = !empty($attachments) || $this->site->getAttachments($id, 'sale');
    ...
    public function getSales($warehouse_id = null)
    {
        ...
        $detail_link       = anchor('admin/sales/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('Détails BL'));
        $duplicate_link    = anchor('admin/sales/add?sale_id=$1', '<i class="fa fa-plus-circle"></i> ' . lang('Dupliquer BL'));
        ...
        $email_link        = anchor('admin/sales/email/$1', '<i class="fa fa-envelope"></i> ' . lang('Envoyer par Email'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link         = anchor('admin/sales/edit/$1', '<i class="fa fa-edit"></i> ' . lang('edit_sale'), 'class="sledit"');
        $convert_link = anchor('admin/factures/add/$1', '<i class="fa fa-file"></i> ' . lang('Créer Facture'));
        $pdf_link          = anchor('admin/sales/pdf/$1', '<i class="fa fa-file-pdf-o"></i> ' . lang('Télécharger BL').' (PDF)');
        $delete_link       = "<a href='#' class='po' title='<b>" . lang('Supprimer Facture') . "</b>' data-content=\"<p>"
        ...
        if ($warehouse_id) {
            ...
            $this->datatables
                ...
                c.company,c. name, sale_status, grand_total, paid, (grand_total-paid) as balance, payment_status, {$this->db->dbprefix('sales')}.attachment, return_id")
                ->from('sales')
    ...
    public function pdf_beta($id = null, $view = null, $save_bufffer = null)
    {
        $this->sma->checkPermissions();
        ...
        $name = lang('sale') . '_' . str_replace('/', '_', $inv->reference_no) . '.pdf';
        $html = $this->load->view($this->theme . 'sales/pdf_beta', $this->data, true);
        if (!$this->Settings->barcode_img) {
            $html = preg_replace("'\<\?xml(.*)\?\>'", '', $html);
        }

        if ($view) {
            $this->load->view($this->theme . 'sales/pdf_beta', $this->data);
        } elseif ($save_bufffer) {
            return $this->sma->generate_pdf($html, $name, $save_bufffer, $this->data['biller']->invoice_footer);
        } else {
            $this->sma->generate_pdf($html, $name, false, $this->data['biller']->invoice_footer);
