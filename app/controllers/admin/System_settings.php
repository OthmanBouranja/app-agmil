...
class system_settings extends MY_Controller
{
    ...
    public function email_templates($template = 'credentials')
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->data['facture']           = file_get_contents('./themes/' . $this->theme . 'email_templates/facture.html');
            $this->data['devis']           = file_get_contents('./themes/' . $this->theme . 'email_templates/devis.html');
    ...
    public function index()
    {
        ...
        $this->form_validation->set_rules('devis_prefix', lang('devis_prefix'), 'trim');
        $this->form_validation->set_rules('quote_prefix', lang('quote_prefix'), 'trim');
        $this->form_validation->set_rules('facture_prefix', lang('facture_prefix'), 'trim');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $data = ['site_name' => DEMO ? 'Stock Manager Advance' : $this->input->post('site_name'),
                ...
                'devis_prefix'         => $this->input->post('devis_prefix'),
    ...
    public function permissions($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            $data = [
                ...
                'sales-pdf_beta'             => $this->input->post('sales-pdf_beta'),
                ...
                'factures-index'               => $this->input->post('factures-index'),
                'factures-edit'                => $this->input->post('factures-edit'),
                'factures-add'                 => $this->input->post('factures-add'),
                'factures-delete'              => $this->input->post('factures-delete'),
                'factures-email'               => $this->input->post('factures-email'),
                'factures-pdf'                 => $this->input->post('factures-pdf'),
                'devis-index'               => $this->input->post('devis-index'),
                'devis-edit'                => $this->input->post('devis-edit'),
                'devis-add'                 => $this->input->post('devis-add'),
                'devis-delete'              => $this->input->post('devis-delete'),
                'devis-email'               => $this->input->post('devis-email'),
                'devis-pdf'                 => $this->input->post('devis-pdf'),
    ...
    public function add_carBrand()
    {
        ...
        } elseif ($this->input->post('add_carBrand')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/categories');
        }

        if ($this->form_validation->run() == true && $this->settings_model->addCarBrand($data)) {
            $this->session->set_flashdata('message', "Marque ajouté");
            admin_redirect('system_settings/carBrand');
        } else {
            $this->data['error']      = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['cardBrands'] = $this->settings_model->getParentCarBrands();
            $this->data['modal_js']   = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_carBrand', $this->data);
        }
    }

    public function getCarBrands()
    {
        $print_barcode = anchor('admin/products/print_barcodes/?carbrand=$1', '<i class="fa fa-print"></i>', 'title="' . lang('print_barcodes') . '" class="tip"');

        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('carbrands')}.id as id, {$this->db->dbprefix('carbrands')}.image, {$this->db->dbprefix('carbrands')}.code, {$this->db->dbprefix('carbrands')}.name, {$this->db->dbprefix('carbrands')}.slug, c.name as parent", false)
            ->from('carbrands')
            ->join('carbrands c', 'c.id=carbrands.parent_id', 'left')
            ->group_by('carbrands.id')
            ->add_column('Actions', "<div class='text-center'> <a href='" . admin_url('system_settings/edit_carbrand/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='Modifier la marque'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>Supprimer la marque</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_carbrand/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id');

        echo $this->datatables->generate();
    }

    public function edit_carbrand($id = null)
    {
        $this->load->helper('security');
        $this->form_validation->set_rules('code', lang('category_code'), 'trim|required');
        $pr_details = $this->settings_model->getCarBrandByID($id);
        if ($this->input->post('code') != $pr_details->code) {
            $this->form_validation->set_rules('code', lang('category_code'), 'required|is_unique[categories.code]');
        }
        $this->form_validation->set_rules('slug', lang('slug'), 'required|alpha_dash');
        ...
        } elseif ($this->input->post('edit_carbrand')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/carBrand');
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateCarBrand($id, $data)) {
            $this->session->set_flashdata('message', "Marque modfiée");
            admin_redirect('system_settings/carBrand');
        } else {
            $this->data['error']      = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['carbrand']   = $this->settings_model->getCarBrandByID($id);
            $this->data['carbrands'] = $this->settings_model->getParentCarBrands();
            $this->data['modal_js']   = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_carbrand', $this->data);
        }
    }
    ...
    public function carBrand()
    {
        ...
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('categories')]];
        $meta                = ['page_title' => lang('categories'), 'bc' => $bc];
        $this->page_construct('settings/carBrand', $meta, $this->data);
    }

    public function delete_carbrand($id = null)
    {
        if (!$id) {
            $this->sma->send_json(['error' => 1, 'msg' => lang('id_not_found')]);
        }
        if ($this->site->getSubCarBrand($id)) {
            $this->sma->send_json(['error' => 1, 'msg' => 'La marque a des sous-marques']);
        }

        if ($this->settings_model->deleteCarBrand($id)) {
            $this->sma->send_json(['error' => 0, 'msg' => 'Marque bien supprimé']);
        }
    }

    public function carBrandsuggestions($term = null, $limit = null, $a = null)
    {
        $this->load->admin_model('settings_model');
        ...
        $result = $this->settings_model->getCarBrandSuggestions($term, $limit);
        if ($a) {
    ...
    {
        // $this->sma->checkPermissions('index');
        $row = $this->settings_model->getCarBrand($id);
        $this->sma->send_json([['id' => $row->id, 'text' => ($row->company && $row->company != '-' ? $row->company : $row->name)]]);
    }

    public function getCarBrand($id = null)
    {
        // $this->sma->checkPermissions('index');
        $row = $this->settings_model->getCarBrandByID($id);
    ...
    public function add_carModel()
    {
        ...
        } elseif ($this->input->post('add_carModel')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/categories');
        }

        if ($this->form_validation->run() == true && $this->settings_model->addCarModel($data)) {
            $this->session->set_flashdata('message', "Marque ajouté");
            admin_redirect('system_settings/carModel');
        } else {
            $this->data['error']      = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['cardBrands'] = $this->settings_model->getParentCarModels();
            $this->data['modal_js']   = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_carModel', $this->data);
        }
    }

    public function getCarModels()
    {
        $print_barcode = anchor('admin/products/print_barcodes/?carmodel=$1', '<i class="fa fa-print"></i>', 'title="' . lang('print_barcodes') . '" class="tip"');

        $this->load->library('datatables');
        $this->datatables
            ->select("{$this->db->dbprefix('carmodels')}.id as id, {$this->db->dbprefix('carmodels')}.image, {$this->db->dbprefix('carmodels')}.code, {$this->db->dbprefix('carmodels')}.name, {$this->db->dbprefix('carmodels')}.slug, c.name as parent", false)
            ->from('carmodels')
            ->join('carmodels c', 'c.id=carmodels.parent_id', 'left')
            ->group_by('carmodels.id')
            ->add_column('Actions', "<div class='text-center'> <a href='" . admin_url('system_settings/edit_carmodel/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='Modifier le modèle'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>Supprimer la marque</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_carmodel/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id');
    ...
    public function edit_carmodel($id = null)
    {
        $this->load->helper('security');
        $this->form_validation->set_rules('code', lang('category_code'), 'trim|required');
        $pr_details = $this->settings_model->getCarModelByID($id);
        if ($this->input->post('code') != $pr_details->code) {
            $this->form_validation->set_rules('code', lang('category_code'), 'required|is_unique[categories.code]');
        }
        $this->form_validation->set_rules('slug', lang('slug'), 'required|alpha_dash');
        ...
        } elseif ($this->input->post('edit_carmodel')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/carModel');
        }

        if ($this->form_validation->run() == true && $this->settings_model->updateCarModel($id, $data)) {
            $this->session->set_flashdata('message', "Marque modfiée");
            admin_redirect('system_settings/carModel');
        } else {
            $this->data['error']      = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['carmodel']   = $this->settings_model->getCarModelByID($id);
            $this->data['carmodels'] = $this->settings_model->getParentCarModels();
            $this->data['modal_js']   = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/edit_carmodel', $this->data);
        }
    }
    ...
    public function carModel()
    {
        ...
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => lang('categories')]];
        $meta                = ['page_title' => lang('categories'), 'bc' => $bc];
        $this->page_construct('settings/carModel', $meta, $this->data);
    }

    public function delete_carmodel($id = null)
    {
        if (!$id) {
            $this->sma->send_json(['error' => 1, 'msg' => lang('id_not_found')]);
        }
        if ($this->site->getSubCarModel($id)) {
            $this->sma->send_json(['error' => 1, 'msg' => 'Le modèle a des sous-modèles']);
        }

        if ($this->settings_model->deleteCarModel($id)) {
            $this->sma->send_json(['error' => 0, 'msg' => 'Modèle bien supprimé']);
        }
    }


    public function add_customerCard()
    {
    ...
    {
        $print_barcode = anchor('admin/products/print_barcodes/?customercard=$1', '<i class="fa fa-print"></i>', 'title="' . lang('print_barcodes') . '" class="tip"');

        $this->load->library('datatables');
        $this->datatables
            ->select('id, name,customer_group_name, company,gst_no ,address,city')
            ->from('customer_card')
            ->add_column('Actions', "<div class='text-center'> <a href='" . admin_url('system_settings/edit_customercard/$1') . "' class='tip' title='Modifier la fiche'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>Supprimer la fiche</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_customercard/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", 'id');
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc                  = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings'), 'page' => lang('system_settings')], ['link' => '#', 'page' => 'Fiches client']];
        $meta                = ['page_title' => 'Fiches client', 'bc' => $bc];
        $this->page_construct('settings/customerCard', $meta, $this->data);
    }

    public function delete_customercard($id = null)
    {
        if (!$id) {
            $this->sma->send_json(['error' => 1, 'msg' => lang('id_not_found')]);
        }

        if ($this->settings_model->deleteCustomerCard($id)) {
            $this->sma->send_json(['error' => 0, 'msg' => 'Fiche client bien supprimée']);
        }
    }
    ...
    public function addCustomerCard()
    {
        ...
        } elseif ($this->input->post('add_customerCard')) {
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect('system_settings/customerCard');
        }

        if ($this->settings_model->addCustomerCard($data)) {
            $this->session->set_flashdata('message', 'Fiche client ajouté');
            admin_redirect('system_settings/customerCard');
        } else {
            $this->data['error']           = (validation_errors() ? validation_errors() : $this->session->flashdata('error'));
            $this->data['modal_js']        = $this->site->modal_js();
            ...
            $bc                                = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings/customerCard'), 'page' => lang('Fiches client')], ['link' => '#', 'page' => lang('Ajouter fiche client')]];
            $meta                              = ['page_title' => lang('Ajouter fiche client'), 'bc' => $bc];
            $this->page_construct('settings/add_customerCard', $meta, $this->data);
        }
    }

    public function edit_customercard($id = null)
    {
        $this->sma->checkPermissions(false, true);
        ...
        if ($this->form_validation->run('customerCard/edit') == true) {
            $cg   = $this->site->getCustomerGroupByID($this->input->post('customer_group'));
            $pg   = $this->site->getPriceGroupByID($this->input->post('price_group'));
            $data = [
                'name'                => $this->input->post('name'),
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updateCustomerCard($id, $data)) {
            $this->session->set_flashdata('message', "Fiche client modifiée");
            admin_redirect('system_settings/customerCard');
        } else {
            $this->data['error']      = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['customer_groups'] = $this->companies_model->getAllCustomerGroups();
            $this->data['customercard']   = $this->settings_model->getCustomerCardByID($id);
            $this->data['modal_js']   = $this->site->modal_js();
            $bc                                = [['link' => base_url(), 'page' => lang('home')], ['link' => admin_url('system_settings/customerCard'), 'page' => lang('Fiches client')], ['link' => '#', 'page' => lang('Modifier fiche client')]];
            $meta                              = ['page_title' => lang('Modifier fiche client'), 'bc' => $bc];
            $this->page_construct('settings/edit_customerCard', $meta, $this->data);
        }
    }

    public function view_customercard($id = null)
    {
        $this->sma->checkPermissions('index', true);
        $this->data['error']    = (validation_errors()) ? validation_errors() : $this->session->flashdata('error');
        $this->data['customercard']   = $this->settings_model->getCustomerCardByID($id);
        $this->load->view($this->theme . 'settings/view_customercard', $this->data);
    }

    public function getModelsByBrand($carBrandId = null)
    {
        if ($rows = $this->settings_model->getModelsByBrand($carBrandId)) {
