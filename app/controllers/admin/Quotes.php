...
class Quotes extends MY_Controller
{
    ...
    public function add($devis_id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $quassurance             = $this->input->post('quassurance');
            $qucourtier             = $this->input->post('qucourtier');
            $qusinistre             = $this->input->post('qusinistre');
            ...
            $data           = ['date' => $date,
                ...
                'supplier'            => $supplier,
                'warehouse_id'        => $warehouse_id,
                'quassurance'         => $quassurance,
                'qucourtier'          => $qucourtier,
                'qusinistre'          => $qusinistre,
                'note'                => $note,
                'total'               => $total,
            ...
            if($this->quotes_model->getByDevis($devis)){
                $this->session->set_flashdata('error', 'Référence de devis déjà utilisée');
                redirect($_SERVER['HTTP_REFERER']);
            }
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->addQuote($data, $products,$attachments)) {
            ...
            if ($devis_id) {
                ...
                $this->data['devis'] = $this->devis_model->getDevisByID($devis_id);
                $items               = $this->devis_model->getAllDevisItems($devis_id);
                krsort($items);
                ...
                $this->data['devis_items'] = json_encode($pr);
            }
            ...
            $this->data['devis_id']   = $devis_id ? $devis_id : "";
    ...
    public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $attachments        = $this->attachments->upload();
            $data['attachment'] = !empty($attachments) || $this->site->getAttachments($id, 'quote');
    ...
    public function getQuotes($warehouse_id = null)
    {
        ...
        $detail_link  = anchor('admin/quotes/view/$1', '<i class="fa fa-file-text-o"></i> ' . lang('Détails de OR'));
        $email_link   = anchor('admin/quotes/email/$1', '<i class="fa fa-envelope"></i> ' . lang('Envoyer par Email'), 'data-toggle="modal" data-target="#myModal"');
        $edit_link    = anchor('admin/quotes/edit/$1', '<i class="fa fa-edit"></i> ' . lang('Modifier OR'));
        $convert_link = anchor('admin/sales/add/$1', '<i class="fa fa-file"></i> ' . lang('Transférer BL'));
        $pc_link      = anchor('admin/purchases/add/$1', '<i class="fa fa-star"></i> ' . lang('Créer Achat'));
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select('id, date, reference_no, biller, customer, supplier,datediff(CURDATE(), date) as deadline, grand_total, status, attachment')
                ->from('quotes')
                ->where('warehouse_id', $warehouse_id);
        } else {
            $this->datatables
                ->select("{$this->db->dbprefix('quotes')}.id as id, {$this->db->dbprefix('quotes')}.date, reference_no, biller, 
               c.company,c.name,
                datediff(CURDATE(), date) as deadline, grand_total, status, {$this->db->dbprefix('quotes')}.attachment")
                ->from('quotes')
                ->join('companies c','c.id = quotes.customer_id','left');
        }
    ...
    public function index($warehouse_id = null)
    {
        ...
        $userReportSynthesis = $this->quotes_model->userReportSynthesis();


        $this->data['userReportSynthesis'] = $userReportSynthesis;
    ...
    public function modal_view($quote_id = null)
    {
        ...
        $this->data['warehouse']  = $this->site->getWarehouseByID($inv->warehouse_id);
        $this->data['inv']        = $inv;
        $this->data['attachments'] = $this->site->getAttachments($quote_id, 'quote');
